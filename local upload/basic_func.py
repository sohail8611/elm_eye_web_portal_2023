...
# server = "http://185.230.211.145:1244"
# def db_connection():
...
def get_list_of_streets(city_name):
    streets_list = []
    cnx = db_connection()
    cursor = cnx.cursor()
    query = (
        "SELECT `streetid`, `streetname` FROM `street` WHERE `city`='"+city_name+"' ORDER BY `streetid` ASC;")
    cursor.execute(query)
    for a, b in cursor:
        streets_list.append({
            'id': a,
...
def upload_new_street_name(name, city_name):
    cnx = db_connection()
    cursor = cnx.cursor()
    query = (
        "INSERT INTO `street`(`streetname`, `in_use`,`city`) VALUES ('"+name+"',0,'"+city_name+"');")
    cursor.execute(query)
    cnx.commit()

    cursor_ = cnx.cursor()
    query = (
        "SELECT `streetid`  FROM `street` WHERE `streetname` = '"+name+"' and `city`='"+city_name+"';")
    cursor_.execute(query)
    st_id = 0
    for a in cursor_:
        st_id = a
...
def insert_violations_Street(violations):
    ...
    for j in violations:
        ...
        query_data_list.append("('" + str(j['street_id']) + "','" + str(
            ...
            j['violation_time']) + "','0','0','No',"+str(j['correct'])+",'"+str(j['super_violation_id'])+"','"+str(j['operation_id'])+"',-1)")
    ...
    cursor = cnx.cursor()
    query = (
                "INSERT INTO `violation`(`street_id`, `violation_type_id`, `details`, `accurate`, `risk`, `display_img`, `lat`, `long`,`device_id`, `polygon_img`, `violation_date`, `violation_time`, `violation_status`, `action_taken`,`api_called`, `correct`, `super_violation_id`,`operation_id`,`sensitivity`) VALUES " +
                query0)
...
def insert_invalid_violations(violations):
    query_data_list = []

    for j in violations:
        query_data_list.append("('" + str(j['operation_id']) + "','" + str(
            j['frame_id']) + "','" + str(j['case_type']) + "','" + str(
            j['accurate']) + "','" + str(j['track_id'])+ "','" + str(
            j['_date'])+ "','" + str(
            j['_time']) + "','" + str(
            j['lat']) + "','" + str(
            j['lng']) + "',\"" + str(j['string_img']) + "\",'" + str(
            j['city']) + "' )")
    ...
    cursor = cnx.cursor()
    query = (
            "INSERT INTO `invalid_violations`(`operation_id`, `frame_id`, `case_type`, `confidence`, `track_id`, `date`, `time`, `latitude`, `longitude`, `img`, `city`) VALUES " +
            query0)
