...
def set_street_id_name_viotype_correct_to_violations(list_of_all_violations, city_name):
    set_of_used_street_id = set()
    list_of_all_streets = get_list_of_streets(city_name)
...
def main_function(list_of_all_violations, date_of_violation, city_name, invalid_violation):
    ...
    list_of_all_violations, list_of_incorrect_violations, set_of_used_street_id = set_street_id_name_viotype_correct_to_violations(
        list_of_all_violations, city_name)
    ...
    print("Number of Invalid Violations: ", len(invalid_violation))
    ...
    for i in range(len(list_of_ava_violations)):
        if list_of_ava_violations[i]['street_id'] != 0 and list_of_ava_violations[i]['street_name'] != "None" and int(list_of_ava_violations[i]['violation_type_id']) != 0:
            super_ids = "0"
    ...
    if len(invalid_violation) > 0:
        insert_invalid_violations(invalid_violation)
